...
def generate_test_script(folder, language, num_tests, problem):
    ...
    with open(folder + 'stress.sh', 'w') as test:
        test.write(
            '#!/usr/bin/env bash\n'
            'g++ -o $3 $3.cc\n'
            'g++ -o $2 $2.cc\n'
            ...
            'H1=`md5sum outWrong`\n'
            'H2=`md5sum outSlow`\n'
            'if !(diff -Z outWrong outSlow > /dev/null)\n'
            'then\n'
                'echo "Error found!"\n'
                'echo "Input:"\n'
                'cat input\n'
    ...
    with open(folder + 'test.sh', 'w') as test:
        ...
        test.write(
            ...
            '    fi\n'
            'done\n'
            'if [ $accepted -eq $tt ]; then\n'
            '   echo {1}{3}$accepted / $tt tests passed!{2} {6}\n'
            '   echo \n'
            'else\n'
            '   echo {1}{4}$accepted / $tt tests passed!{2} {6}\n'
            '   echo \n'
            'fi\n'
            .format(num_tests, BOLD, NORM, GREEN_F, RED_F, TIME_CMD, TIME_AP, run_cmd=param["RUN_CMD"]))
...
def main():
    ...
    for index, problem in enumerate(content.problems):
        ...
        directory = os.getcwd()
        os.chdir(directory + "/" + contest + "-c++14/" + problem)
        os.system("subl " + problem + ".cc")
        os.chdir(directory)
